...
module.exports = {
  getIndex,
  postIndex, //by index:merge or diff:merge
...
async function getIndex(project_id, index_id, isDiff) {
  try {
...
async function postIndex(project_id, payload, isDiff) {
  try {
    const endpoint = isDiff ?
      config.IndexV2API.post_diff_index.format(project_id) :
      config.IndexV2API.post_index.format(project_id)
    const headers = config.httpHeaders(config.credentials.token_3legged)
...
async function postIndexBatchStatus(project_id, payload, isDiff) {
  try {
    const endpoint = isDiff ?
...
async function getQuery(project_id, index_id, query_id, isDiff) {
  try {
    const endpoint = isDiff ?
...
async function postQuery(project_id, payload, isDiff) {
  try {
    const endpoint = isDiff ?
      config.IndexV2API.post_diff_query.format(project_id) :
      config.IndexV2API.post_query.format(project_id)
    const headers = config.httpHeaders(config.credentials.token_3legged)
    const response = await post(endpoint, headers, payload);
...
async function getIndexManifest(project_id, index_id, isDiff) {
  try {
    const endpoint = isDiff ?
...
async function getIndexFields(project_id, index_id, isDiff,path, filename) {
  try {
    const endpoint = isDiff ?
...
async function getIndexProperties(project_id, index_id, isDiff, path, filename) {
  try {
    const endpoint = isDiff ?
